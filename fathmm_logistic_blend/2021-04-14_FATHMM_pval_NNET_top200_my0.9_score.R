...
fathmm_top200_train = read.csv("fathmm_pval_top_200_scores_train.csv", header = T)
fathmm_200 = read.table("fathmm_pval_top200.tsv", sep = "\t", header = T)
fathmm_200$SNP_ALT = paste0("X", fathmm_200$ID, "_", fathmm_200$variant)
fathmm_200$SNP_ALT = str_replace(fathmm_200$SNP_ALT, ":", ".")

cor_matrix = cor(fathmm_top200_train, method = "pearson")
...
for (i in c(1:length(pairs))) {
  score1 = fathmm_200[str_detect(pairs[[i]][1], fathmm_200$SNP_ALT),6]
  score2 = fathmm_200[str_detect(pairs[[i]][2], fathmm_200$SNP_ALT),6]
...

{
  if (length(columns_to_remove) > 0) {
    simplified_top200_train = fathmm_top200_train[,-which(names(fathmm_top200_train) %in% columns_to_remove)]
  }
  else {
    simplified_top200_train = fathmm_top200_train
  }
}



fathmm_top200_test = read.csv("fathmm_pval_top_200_scores_test.csv", header = T)



{
  if (length(columns_to_remove) > 0) {
    simplified_top200_test = fathmm_top200_test[,-which(names(fathmm_top200_test) %in% columns_to_remove)]
  }
  else {
    simplified_top200_test = fathmm_top200_test
  }
}



write.table(simplified_top200_train, "simplified_fathmm_pval_top200_my0.9_SCORES_train.csv", row.names = F, quote = F, sep = ",")
write.table(simplified_top200_test, "simplified_fathmm_pval_top200_my0.9_SCORES_test.csv", row.names = F, quote = F, sep = ",")


set.seed(200)

train = simplified_top200_train
x_train = model.matrix(PHENOTYPE ~ ., train)[,c(-1)]
y_train = as.factor(train[, 2])

test = simplified_top200_test
...
ctrl <- trainControl(method = "repeatedcv",   # 200 fold cross validation with 3x repeats
                     repeats = 3)
...
saveRDS(tuned_nnet, "./tuned_nnet_fathmm_pval_top200_mycorr0.9_score.rds")