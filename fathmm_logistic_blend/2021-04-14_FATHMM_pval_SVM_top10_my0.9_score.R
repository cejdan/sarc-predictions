...
fathmm_top10_train = read.csv("fathmm_pval_top_10_scores_train.csv", header = T)
fathmm_10 = read.table("fathmm_pval_top10.tsv", sep = "\t", header = T)
fathmm_10$SNP_ALT = paste0("X", fathmm_10$ID, "_", fathmm_10$variant)
fathmm_10$SNP_ALT = str_replace(fathmm_10$SNP_ALT, ":", ".")

cor_matrix = cor(fathmm_top10_train, method = "pearson")
...
for (i in c(1:length(pairs))) {
  score1 = fathmm_10[str_detect(pairs[[i]][1], fathmm_10$SNP_ALT),6]
  score2 = fathmm_10[str_detect(pairs[[i]][2], fathmm_10$SNP_ALT),6]
...

{
  if (length(columns_to_remove) > 0) {
    simplified_top10_train = fathmm_top10_train[,-which(names(fathmm_top10_train) %in% columns_to_remove)]
  }
  else {
    simplified_top10_train = fathmm_top10_train
  }
}



fathmm_top10_test = read.csv("fathmm_pval_top_10_scores_test.csv", header = T)



{
  if (length(columns_to_remove) > 0) {
    simplified_top10_test = fathmm_top10_test[,-which(names(fathmm_top10_test) %in% columns_to_remove)]
  }
  else {
    simplified_top10_test = fathmm_top10_test
  }
}



write.table(simplified_top10_train, "simplified_fathmm_pval_top10_my0.9_SCORES_train.csv", row.names = F, quote = F, sep = ",")
write.table(simplified_top10_test, "simplified_fathmm_pval_top10_my0.9_SCORES_test.csv", row.names = F, quote = F, sep = ",")
...
saveRDS(tuned_svm, "./tuned_svmRadial_fathmm_pval_top10_mycorr0.9_score.rds")