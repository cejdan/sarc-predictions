...
fathmm_top1000_train = read.csv("fathmm_pval_top_1000_scores_train.csv", header = T)
fathmm_1000 = read.table("fathmm_pval_top1000.tsv", sep = "\t", header = T)
fathmm_1000$SNP_ALT = paste0("X", fathmm_1000$ID, "_", fathmm_1000$variant)
fathmm_1000$SNP_ALT = str_replace(fathmm_1000$SNP_ALT, ":", ".")

cor_matrix = cor(fathmm_top1000_train, method = "pearson")
...
for (i in c(1:length(pairs))) {
  score1 = fathmm_1000[str_detect(pairs[[i]][1], fathmm_1000$SNP_ALT),6]
  score2 = fathmm_1000[str_detect(pairs[[i]][2], fathmm_1000$SNP_ALT),6]
...

{
  if (length(columns_to_remove) > 0) {
    simplified_top1000_train = fathmm_top1000_train[,-which(names(fathmm_top1000_train) %in% columns_to_remove)]
  }
  else {
    simplified_top1000_train = fathmm_top1000_train
  }
}



fathmm_top1000_test = read.csv("fathmm_pval_top_1000_scores_test.csv", header = T)



{
  if (length(columns_to_remove) > 0) {
    simplified_top1000_test = fathmm_top1000_test[,-which(names(fathmm_top1000_test) %in% columns_to_remove)]
  }
  else {
    simplified_top1000_test = fathmm_top1000_test
  }
}



write.table(simplified_top1000_train, "simplified_fathmm_pval_top1000_my0.9_SCORES_train.csv", row.names = F, quote = F, sep = ",")
write.table(simplified_top1000_test, "simplified_fathmm_pval_top1000_my0.9_SCORES_test.csv", row.names = F, quote = F, sep = ",")
...
ctrl <- trainControl(method = "repeatedcv",   # 1000 fold cross validation with 3x repeats
                     repeats = 3)
...
saveRDS(tuned_nnet, "./tuned_nnet_fathmm_pval_top1000_mycorr0.9_score.rds")