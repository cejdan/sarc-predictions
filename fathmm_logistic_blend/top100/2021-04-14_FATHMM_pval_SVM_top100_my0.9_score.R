...
fathmm_top100_train = read.csv("fathmm_pval_top_100_scores_train.csv", header = T)
fathmm_100 = read.table("fathmm_pval_top100.tsv", sep = "\t", header = T)
fathmm_100$SNP_ALT = paste0("X", fathmm_100$ID, "_", fathmm_100$variant)
fathmm_100$SNP_ALT = str_replace(fathmm_100$SNP_ALT, ":", ".")

cor_matrix = cor(fathmm_top100_train, method = "pearson")
...
for (i in c(1:length(pairs))) {
  score1 = fathmm_100[str_detect(pairs[[i]][1], fathmm_100$SNP_ALT),6]
  score2 = fathmm_100[str_detect(pairs[[i]][2], fathmm_100$SNP_ALT),6]
...

{
  if (length(columns_to_remove) > 0) {
    simplified_top100_train = fathmm_top100_train[,-which(names(fathmm_top100_train) %in% columns_to_remove)]
  }
  else {
    simplified_top100_train = fathmm_top100_train
  }
}



fathmm_top100_test = read.csv("fathmm_pval_top_100_scores_test.csv", header = T)



{
  if (length(columns_to_remove) > 0) {
    simplified_top100_test = fathmm_top100_test[,-which(names(fathmm_top100_test) %in% columns_to_remove)]
  }
  else {
    simplified_top100_test = fathmm_top100_test
  }
}



write.table(simplified_top100_train, "simplified_fathmm_pval_top100_my0.9_SCORES_train.csv", row.names = F, quote = F, sep = ",")
write.table(simplified_top100_test, "simplified_fathmm_pval_top100_my0.9_SCORES_test.csv", row.names = F, quote = F, sep = ",")
...
saveRDS(tuned_svm, "./tuned_svmRadial_fathmm_pval_top100_mycorr0.9_score.rds")