...
cadd_top200_train = read.csv("cadd_pval_top_200_scores_train.csv", header = T)
cadd_200 = read.table("cadd_pval_top200.tsv", sep = "\t", header = T)
cadd_200$SNP_ALT = paste0("X", cadd_200$SNP, "_", cadd_200$ALT)
cadd_200$SNP_ALT = str_replace(cadd_200$SNP_ALT, ":", ".")

cor_matrix = cor(cadd_top200_train, method = "pearson")
...
for (i in c(1:length(pairs))) {
  score1 = cadd_200[str_detect(pairs[[i]][1], cadd_200$SNP_ALT),6]
  score2 = cadd_200[str_detect(pairs[[i]][2], cadd_200$SNP_ALT),6]
...

{
  if (length(columns_to_remove) > 0) {
    simplified_top200_train = cadd_top200_train[,-which(names(cadd_top200_train) %in% columns_to_remove)]
  }
  else {
    simplified_top200_train = cadd_top200_train
  }
}



cadd_top200_test = read.csv("cadd_pval_top_200_scores_test.csv", header = T)



{
  if (length(columns_to_remove) > 0) {
    simplified_top200_test = cadd_top200_test[,-which(names(cadd_top200_test) %in% columns_to_remove)]
  }
  else {
    simplified_top200_test = cadd_top200_test
  }
}



write.table(simplified_top200_train, "simplified_caddpval_top200_my0.9_SCORES_train.csv", row.names = F, quote = F, sep = ",")
write.table(simplified_top200_test, "simplified_caddpval_top200_my0.9_SCORES_test.csv", row.names = F, quote = F, sep = ",")
...
saveRDS(tuned_forest, "./tuned_forest_cadd_pval_top200_mycorr0.9_score.rds")
