...
cadd_top500_train = read.csv("cadd_pval_top_500_scores_train.csv", header = T)
cadd_500 = read.table("cadd_pval_top500.tsv", sep = "\t", header = T)
cadd_500$SNP_ALT = paste0("X", cadd_500$SNP, "_", cadd_500$ALT)
cadd_500$SNP_ALT = str_replace(cadd_500$SNP_ALT, ":", ".")

cor_matrix = cor(cadd_top500_train, method = "pearson")
...
for (i in c(1:length(pairs))) {
  score1 = cadd_500[str_detect(pairs[[i]][1], cadd_500$SNP_ALT),6]
  score2 = cadd_500[str_detect(pairs[[i]][2], cadd_500$SNP_ALT),6]
...

{
  if (length(columns_to_remove) > 0) {
    simplified_top500_train = cadd_top500_train[,-which(names(cadd_top500_train) %in% columns_to_remove)]
  }
  else {
    simplified_top500_train = cadd_top500_train
  }
}



cadd_top500_test = read.csv("cadd_pval_top_500_scores_test.csv", header = T)



{
  if (length(columns_to_remove) > 0) {
    simplified_top500_test = cadd_top500_test[,-which(names(cadd_top500_test) %in% columns_to_remove)]
  }
  else {
    simplified_top500_test = cadd_top500_test
  }
}



write.table(simplified_top500_train, "simplified_caddpval_top500_my0.9_SCORES_train.csv", row.names = F, quote = F, sep = ",")
write.table(simplified_top500_test, "simplified_caddpval_top500_my0.9_SCORES_test.csv", row.names = F, quote = F, sep = ",")
...
saveRDS(tuned_forest, "./tuned_forest_cadd_pval_top500_mycorr0.9_score.rds")
