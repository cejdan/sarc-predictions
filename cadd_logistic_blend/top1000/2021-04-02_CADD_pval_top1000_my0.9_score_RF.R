...
cadd_top1000_train = read.csv("cadd_pval_top_1000_scores_train.csv", header = T)
cadd_1000 = read.table("cadd_pval_top1000.tsv", sep = "\t", header = T)
cadd_1000$SNP_ALT = paste0("X", cadd_1000$SNP, "_", cadd_1000$ALT)
cadd_1000$SNP_ALT = str_replace(cadd_1000$SNP_ALT, ":", ".")

cor_matrix = cor(cadd_top1000_train, method = "pearson")
...
for (i in c(1:length(pairs))) {
  score1 = cadd_1000[str_detect(pairs[[i]][1], cadd_1000$SNP_ALT),6]
  score2 = cadd_1000[str_detect(pairs[[i]][2], cadd_1000$SNP_ALT),6]
...

{
  if (length(columns_to_remove) > 0) {
    simplified_top1000_train = cadd_top1000_train[,-which(names(cadd_top1000_train) %in% columns_to_remove)]
  }
  else {
    simplified_top1000_train = cadd_top1000_train
  }
}



cadd_top1000_test = read.csv("cadd_pval_top_1000_scores_test.csv", header = T)



{
  if (length(columns_to_remove) > 0) {
    simplified_top1000_test = cadd_top1000_test[,-which(names(cadd_top1000_test) %in% columns_to_remove)]
  }
  else {
    simplified_top1000_test = cadd_top1000_test
  }
}



write.table(simplified_top1000_train, "simplified_caddpval_top1000_my0.9_SCORES_train.csv", row.names = F, quote = F, sep = ",")
write.table(simplified_top1000_test, "simplified_caddpval_top1000_my0.9_SCORES_test.csv", row.names = F, quote = F, sep = ",")
...
saveRDS(tuned_forest, "./tuned_forest_cadd_pval_top1000_mycorr0.9_score.rds")
