...
cadd_top10_train = read.csv("cadd_pval_top_10_scores_train.csv", header = T)
cadd_10 = read.table("cadd_pval_top10.tsv", sep = "\t", header = T)
cadd_10$SNP_ALT = paste0("X", cadd_10$SNP, "_", cadd_10$ALT)
cadd_10$SNP_ALT = str_replace(cadd_10$SNP_ALT, ":", ".")

cor_matrix = cor(cadd_top10_train, method = "pearson")
...
for (i in c(1:length(pairs))) {
  score1 = cadd_10[str_detect(pairs[[i]][1], cadd_10$SNP_ALT),6]
  score2 = cadd_10[str_detect(pairs[[i]][2], cadd_10$SNP_ALT),6]
...

{
  if (length(columns_to_remove) > 0) {
    simplified_top10_train = cadd_top10_train[,-which(names(cadd_top10_train) %in% columns_to_remove)]
  }
  else {
    simplified_top10_train = cadd_top10_train
  }
}



cadd_top10_test = read.csv("cadd_pval_top_10_scores_test.csv", header = T)



{
  if (length(columns_to_remove) > 0) {
    simplified_top10_test = cadd_top10_test[,-which(names(cadd_top10_test) %in% columns_to_remove)]
  }
  else {
    simplified_top10_test = cadd_top10_test
  }
}



write.table(simplified_top10_train, "simplified_caddpval_top10_my0.9_SCORES_train.csv", row.names = F, quote = F, sep = ",")
write.table(simplified_top10_test, "simplified_caddpval_top10_my0.9_SCORES_test.csv", row.names = F, quote = F, sep = ",")
...
saveRDS(tuned_forest, "./tuned_forest_cadd_pval_top10_mycorr0.9_score.rds")
