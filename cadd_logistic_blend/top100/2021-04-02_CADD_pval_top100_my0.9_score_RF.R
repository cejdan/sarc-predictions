...
cadd_top100_test = read.csv("cadd_pval_top_100_scores_test.csv", header = T)



{
  if (length(columns_to_remove) > 0) {
    simplified_top100_test = cadd_top100_test[,-which(names(cadd_top100_test) %in% columns_to_remove)]
  }
  else {
    simplified_top100_test = cadd_top100_test
  }
}



write.table(simplified_top100_train, "simplified_caddpval_top100_my0.9_SCORES_train.csv", row.names = F, quote = F, sep = ",")
write.table(simplified_top100_test, "simplified_caddpval_top100_my0.9_SCORES_test.csv", row.names = F, quote = F, sep = ",")
...
saveRDS(tuned_forest, "./tuned_forest_cadd_pval_top100_mycorr0.9_score.rds")
