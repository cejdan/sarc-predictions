...
write.table(lasso_simple, "simplified_lasso_0.9_train.csv", row.names = F, quote = F, sep = ",")
write.table(lasso_simple_test, "simplified_lasso_0.9_test.csv", row.names = F, quote = F, sep = ",")
...
# Select 15 roughly evenly spaced numbers between 1 and the number of columns used in the train dataframe.
# but round decimal values down to the nearest integer
hyper_grid = expand.grid(
  mtry = seq(from = 1, to = length(colnames(train)), by = length(colnames(train))/15) %>% floor()
)
...
saveRDS(tuned_forest, "./tuned_forest_lasso_mycorr0.9.rds")
